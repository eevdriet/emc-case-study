...
def process_data(type: str) -> None:
    ...
    dropped_rows_per_group = data.groupby(['scenario', 'simulation', 'treat_time']).apply(
        lambda x: len(x) - x['post'].count()).reset_index()
    ...
    grouped = pd.merge(grouped, dropped_rows_per_group, on=['scenario', 'simulation', 'treat_time'], how='right')
